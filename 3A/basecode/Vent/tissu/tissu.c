...
void initializeArrayOfParticles(particle ** p_array, float p_width, float p_length, int p_nbPartW, int p_nbPartL, obstacle * p_obstacles, int p_nbObstacles)
{
	...
	for (i = 0; i < p_nbPartW; i++) {	
		for (j = 0; j < p_nbPartL; j++) {
			pos = set3(p_width / p_nbPartW * i, p_length / p_nbPartL * j + 1 , 0.5);
			if ( j == 0 )
			{
				p_array[i][j] = createParticle(n, DEFAULT_MASS, DEFAULT_SIZE, TRUE, pos, set3(0, 0, 0), FRICTION);
			}

			else
			{
				p_array[i][j] = createParticle(n, DEFAULT_MASS, DEFAULT_SIZE, FALSE, pos, set3(0, 0, 0), FRICTION);
				n++;
	...
	for (i = 0; i < p_nbObstacles; i++) {
		...
		p_obstacles[i].m_position = set3(RANDOM_0_1, 0.5, RANDOM_0_1);
		p_obstacles[i].m_size = 150 * DEFAULT_SIZE;
	}		
}
...
particle ** createFabric(float p_width, float p_length, int p_nbPartW, int p_nbPartL, obstacle * p_obstacles, int p_nbObstacles)
{
	...
	initializeArrayOfParticles(array, p_width, p_length, p_nbPartW, p_nbPartL, p_obstacles, 0);
...
void updateParticle(particle * p_particle, float p_dt, obstacle * p_obstacles, int p_nbObstacles)
{
	// TODO 
	srand(time(NULL));
	vec3 forcevent=set3(0,0,RAND(-10,10));
	...
	p_particle->m_speed[NEXT] = add3(v_cur, mul3(add3(forcevent,add3(add3(g, f), forces_voisins)), p_dt));
...
{	
	if (p_particle->m_static == TRUE)
	{
		p_particle->m_position[CURRENT] = p_particle->m_position[CURRENT]; 
		p_particle->m_speed[CURRENT] = set3(0, 0, 0);
	}
	else
	{
