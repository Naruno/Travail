...
// Permet de gerer les astres
vec2f ballePos = {0.0f,0.0f};
vec2f attractPos[NB_ATTRACTEURS];
int attractSetFlag[NB_ATTRACTEURS]= {0}; // Permet de gerer seulement 1 astre pour le bonus 


void sdl_setAttracteurPosition(int index, float x,float y) // Permet de Set la position des astres
{
...
int sdl_loop()
{
    ...
    // On gere la position des astres
    for(i = 0; i <NB_ATTRACTEURS; i++)
    {
        if(attractSetFlag[i])
        {
    ...
    for(i=0; i<NB_BALLES; i++)
    {
        ...
        SDL_Rect dstrect;
        dstrect.x =  WINDOW_SIZE*positions[i].x - bmpBalle->w/2;
        dstrect.y =  WINDOW_SIZE - WINDOW_SIZE*positions[i].y - bmpBalle->h/2;
...
void sdl_clean()
{
    ...
    SDL_FreeSurface(bmpAttracteur); // libere bmpAttracteur
    // all is well ;)
